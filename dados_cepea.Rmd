...
Em que $p_{i,t}$ é o preço da *commodity* $i$ e $i = \text{etanol, açúcar ou soja}$. Percebe-se que a volatilidade do preço etanol é bem mais intensa e com maior amplitude se comparadas às volatilidades do preço do soja e do preço do  açúcar. Entretanto, conforme @schulz_volatility_2016 é característico das séries de preços de *commodities* serem cointegradas e uma medida de volatilidade que leve em conta esta característica dos dados se torna mais apropriada. Para isso pode-se modelar a média da série de preços por meio e um modelo de correção de erros (VECM, sigla em inglês) e então filtrar a série de preços do co-movimento de suas médias condicionais.
...
dados_cepea_deflacionado <- mutate(dados_cepea_deflacionado, ETANOLDF=log(ETANOLDF), SOJADF = log(SOJADF), ACUCARDF = log(ACUCARDF))
plot(dados_cepea_deflacionado$Data,
     dados_cepea_deflacionado$ETANOLDF,
     cex.axis = 1.2,
     cex.lab  = 1.2, 
     type     = "l",
     lwd      = 3,
...
lines(dados_cepea_deflacionado$Data,
      dados_cepea_deflacionado$ACUCARDF,
      type = "l",
      lwd  = 3, 
      lty  = 2)

lines(dados_cepea_deflacionado$Data,
      dados_cepea_deflacionado$SOJADF,
      type = "l",
      lwd  = 4, 
      lty  = 3)
...
dados_cepea <- mutate(dados_cepea, Data = Data, etanol = (etanol-lag(etanol))^2, soja = (soja-lag(soja))^2, acucar = (acucar-lag(acucar))^2)
